...
export const AvatarImage = React.forwardRef<HTMLImageElement, React.ImgHTMLAttributes<HTMLImageElement>>(
  ({ className = '', src, ...props }, ref) => {
    if (!src) return null;
    return <img ref={ref} src={src} className={`aspect-square h-full w-full object-cover ${className}`} {...props} />;
  }
);
...
export const AvatarFallback = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className = '', ...props }, ref) => (
    <div ref={ref} className={`flex h-full w-full items-center justify-center rounded-full bg-slate-100 ${className}`} {...props} />
  )
);
